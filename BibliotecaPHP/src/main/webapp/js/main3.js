...
$(document).ready(function () {
  ...
  // Variables para filtros (si quieres implementarlos)
  let filtroAutor  = '';
  let filtroLibro  = '';

  /** =====================================================
   *  1) CARGAR AUTORES, LIBROS Y RELACIONES
   *  ===================================================== */
  ...
  function loadRelaciones() {
    ...
    .done(function(data) {
      ...
      renderLibrosAutoresTable(); // <=== NUEVA LLAMADA aquí
    })
    .fail(function(xhr) {
      alert('Error al cargar relaciones: ' + xhr.responseText);
      renderLibrosAutoresTable(); // aunque falle, mostramos libros sin autores
    });
  }
  ...
  /** =====================================================
   *  2) POBLAR SELECTS DE LIBROS Y AUTORES (para crear relaciones)
   *  ===================================================== */
  ...
  /** =====================================================
   *  3) RENDERIZAR TABLAS DE AUTORES, LIBROS Y RELACIONES
   *  ===================================================== */
  function renderAutoresTable() {
    const tbody = $('#autoresTable tbody');
    tbody.empty();

    // Aplicar filtro por ID de autor, si existe
    let listaFiltrada = autores;
    if (filtroAutor && filtroAutor.trim() !== '') {
      const valorNum = Number(filtroAutor);
      listaFiltrada = autores.filter(a => a.id_autor === valorNum);
    }
    ...
    if (listaFiltrada.length === 0) {
      tbody.append(
        '<tr><td colspan="4" style="text-align:center">No se encontraron autores</td></tr>'
      );
    }
  }
  ...
  function renderLibrosTable() {
    ...
    // Aplicar filtro por ID o título de libro, si existe
    let listaFiltrada = libros;
    if (filtroLibro && filtroLibro.trim() !== '') {
      const termino = filtroLibro.trim().toLowerCase();
      listaFiltrada = libros.filter(l => {
        if (!isNaN(termino)) {
          return l.id_libro === Number(termino);
        }
        return l.titulo.toLowerCase().includes(termino);
      });
    }
    ...
    if (listaFiltrada.length === 0) {
      tbody.append(
        '<tr><td colspan="3" style="text-align:center">No se encontraron libros</td></tr>'
      );
    }
  }
  ...
  function renderRelacionesTable() {
    ...
    if (relaciones.length === 0) {
      tbody.append(
        '<tr><td colspan="5" style="text-align:center">No hay relaciones</td></tr>'
      );
    }
  }

  function renderLibrosAutoresTable() {
    const tbody = $('#librosAutoresTable tbody');
    tbody.empty();

    // Mapa id_libro → [ nombres de autores ]
    const mapaLibrosAAutores = {};
    relaciones.forEach(r => {
      const idL = r.id_libro;
      const idA = r.id_autor;
      const autorObj = autores.find(a => a.id_autor === idA);
      const nombreCompleto = autorObj ? `${autorObj.nombre} ${autorObj.apellido}`.trim() : '(desconocido)';

      if (!mapaLibrosAAutores[idL]) {
        mapaLibrosAAutores[idL] = [];
      }
      mapaLibrosAAutores[idL].push(nombreCompleto);
    });

    // Por cada libro, obtener la lista de autores concatenada
    libros.forEach(l => {
      const listaNombres = mapaLibrosAAutores[l.id_libro] || [];
      const autoresConcatenados = listaNombres.length > 0
        ? listaNombres.join(', ')
    ...
    if (libros.length === 0) {
      tbody.append(
        '<tr><td colspan="2" style="text-align:center">No se encontraron libros</td></tr>'
      );
    }
  }
  ...
  /** =====================================================
   *  4) EVENTOS DE BÚSQUEDA PARA AUTORES Y LIBROS
   *  ===================================================== */
  $('#searchAutor').on('keyup', function () {
    filtroAutor = $(this).val();
    renderAutoresTable();
  });

  $('#searchLibro').on('keyup', function () {
    filtroLibro = $(this).val();
    renderLibrosTable();
  });


  /** =====================================================
   *  5) EVENTOS PARA AUTORES (CREAR / EDITAR / ELIMINAR)
   *  ===================================================== */
  ...
  $('#autorForm').submit(function (e) {
    ...
    if (!idAutor) {
      ...
      // EDITAR
      payload.id_autor = Number(idAutor);
      $.ajax({
        url: API_AUTORES,
        method: 'PUT',
        contentType: 'application/json',
  ...
  $('#autoresTable').on('click', '.delete-autor', function () {
    ...
    $.ajax({
      ...
      method: 'DELETE',
      contentType: 'application/json',
      data: JSON.stringify({ id_autor: Number(id) }),
      success: function () {
        loadAutores().done(function() { loadRelaciones(); });
      },
      error: function (xhr) {
  ...
  /** =====================================================
   *  6) EVENTOS PARA LIBROS (CREAR / EDITAR / ELIMINAR)
   *  ===================================================== */
  ...
  $('#libroForm').submit(function (e) {
    ...
    if (!idLibro) {
      ...
      // EDITAR
      payload.id_libro = Number(idLibro);
      $.ajax({
        url: API_LIBROS,
        method: 'PUT',
        contentType: 'application/json',
  ...
  $('#librosTable').on('click', '.delete-libro', function () {
    ...
    $.ajax({
      ...
      method: 'DELETE',
      contentType: 'application/json',
      data: JSON.stringify({ id_libro: Number(id) }),
      success: function () {
        loadLibros().done(function() { loadRelaciones(); });
      },
      error: function (xhr) {
